...
class _AuthScreenState extends State<AuthScreen> {
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      GestureDetector(
                        ...
                        child: Container(
                          padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 16),
                          decoration: BoxDecoration(borderRadius: BorderRadius.circular(12), border: _isOtpFocused ? Border.all(color: AppColors.primary, width: 2) : Border.all(color: AppColors.mattedGrey, width: 2), color: AppColors.mattedGrey.withOpacity(0.3)),
                          child: Directionality(
                            textDirection: TextDirection.ltr,
                            child: TextField(
                              controller: _otpController, keyboardType: TextInputType.number, textAlign: TextAlign.center, maxLength: 4,
                              style: const TextStyle(fontSize: 24, color: AppColors.textPrimary, fontFamily: 'Peyda', fontWeight: FontWeight.w700, letterSpacing: 32),
                              cursorColor: AppColors.primary,
                              decoration: const InputDecoration(border: InputBorder.none, counterText: "", hintText: '⁣ − − − −', hintStyle: TextStyle(color: Colors.grey, letterSpacing: 16, fontSize: 20)),
                              onChanged: (code) { if (code.length == 4) _verifyOtp(); },
                            ),
                          ),
                        ),
                      ),
  ...
  void _verifyOtp() async {
    if (_otpController.text.length < 4) { _showAlert('کد تایید ناقص است', isError: true); return; }
    try {
      var response = await http.post(Uri.parse("http://websera.ir/auth.php?action=verify_code"), headers: {'Content-Type': 'application/json'}, body: jsonEncode({"phone": _phoneController.text, "code": _otpController.text}));
      var data = jsonDecode(response.body);
      if (data['status'] == 'success') {
        _setLoggedIn(_phoneController.text); _showAlert('ورود با موفقیت انجام شد', isError: false);
        await Future.delayed(const Duration(seconds: 1));
        if(mounted) Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => const HomeScreen()));
      } else { _showAlert(data['message'] ?? 'کد تایید اشتباه است', isError: true); }
    } catch (e) { _showAlert('خطا در اتصال به سرور', isError: true); }
  }
